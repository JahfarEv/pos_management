...
export const useCategories = () => {
  ...
  const [loadingMoreProducts, setLoadingMoreProducts] = useState<string | null>(
    null
  );
  ...
  const productsBuckets = useAppSelector(
    (state) => state.categories?.productsBuckets || {}
  );
  ...
  const getProductsPagination = useCallback(
    (slug: string) => {
      ...
      return {
        ...
        hasMore:
          (bucket.pagination?.page || 1) < (bucket.pagination?.pages || 1),
  ...
  // ------------------------------------------------------------------
  const loadMoreProductsByCategorySlug = useCallback(
    (slug: string) => {
      const now = Date.now();
      const minDelay = 800;
  ...
  // ------------------------------------------------------------------
  const hasMoreProductsByCategorySlug = useCallback(
    (slug: string) => {
