...
export const CategoryPanel: React.FC<CategoryPanelProps> = ({
  ...
  const handleScroll = useCallback(() => {
    ...
    const triggerRect = loadMoreTriggerRef.current.getBoundingClientRect();
    const windowHeight = window.innerHeight;
    const isNearBottom = triggerRect.top <= windowHeight + (isMobile ? 150 : 300); // Reduced threshold on mobile

    if (isNearBottom && hasMoreCategories()) {
      console.log("Scroll: Near bottom, loading more");
      debouncedLoadMore();
    }
  }, [hasMoreCategories, categoryLoading, loadingMore, debouncedLoadMore, isMobile]);
          ...
          return (
            ...
            >
              {loadingCategory === categoryName ? (
                ...
                <span className="truncate"> {/* Truncate long text */}
                  {categoryName}
                </span>
              )}
            </button>
